...
module {
    ...
    private let IV_256 : [Nat32] = [
        0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
        0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19,
    ];

    private let IV_224 : [Nat32] = [
    ...
    // Calculate a SHA256 hash from Iter.
    public func sha224(iter : Iter.Iter<Nat8>) : Blob {
    ...
    public class Digest(algo: Algorithm) {
        let (word_length, state_words, block_bytes, rounds, sum_words, iv) = 
            switch (algo) {
                ...
                case (_) { // assume sha256 for all unimplemented ones 
                    (#single, 8, 64, 64, 8, IV_256 : [Nat32]); 
                }
            };

        private var len : Nat = 0;
        private let state : [var Nat32] = Array.init<Nat32>(state_words, 0);
        private let buffer = IterExt.BlockBuffer<Nat8>(block_bytes);
        private let rot = Nat32.bitrotRight;

        public func reset() {
            len := 0;
            buffer.reset();
            for (i in Iter.range(0, state_words-1)) { 
                state[i] := iv[i];
            };
        };
        ...
        private let w = Array.init<Nat32>(rounds, 0);
        // TODO need switch statement here over algo
        ...
        public func sum() : Blob {
            ...
            for (i in Iter.range(0, sum_words-1)) {
